...
const EditForm = () => {
  ...
  /*  const loadData = async () => {
    try {
        const res = await axios.get(`http://localhost:5000/api/vehicle/get-record/${id}`)
        if(res.data) {
  ...
  */
  ...
  const handleUpdate = async (values) => {
    try {
      ...
      const res = await axios.put(`http://localhost:5000/api/vehicle/edit-record/${id}`, {...values, userId:user._id})
      dispatch(hideLoading())
      if(res.data.success){
  ...
  return (
    <div className="form-container">
      <div className="form-wrapper">
        <h5 style={{color:"#1aac83", marginBottom: "20px", marginTop:"10px"}}>Edit Record</h5>
        <Form style={{width:"80%"}} layout="vertical" onFinish={handleUpdate} initialValues={records}>
